...
class CharacterVisualComponent: GKComponent {
    ...
    init(type: CharacterList, size: CGSize){
        ...
        case .giant:
            charImage = SKSpriteNode(imageNamed: type.rawValue)
            let desiredSize = CGSize(width: size.width/2, height: size.height) // Set your desired size here
            ...
            charImage.position = CGPoint(x: size.width/2 + charImage.size.width, y: size.height/2)
        case .narrator:
            charImage = SKSpriteNode(imageNamed: type.rawValue)
