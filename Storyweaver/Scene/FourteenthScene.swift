...
class FourteenthScene: SKScene {
    ...
    @ObservedObject private var gameState = GameState(dialogTree: DialogTree.DialogTreeScene14)
...
//        addChild(characterLabel)
        ...
        cutsceneNode = SKSpriteNode(imageNamed: "cutscene_scene14_1")
    ...
    func showNextDialogue() {
        ...
        for case let component as CharacterVisualComponent in characterVisualComponentSytem.components {
            if component.type == .timunMas || component.type == .mbokSrini {
    ...
    func animateTextDisplay(dialogueText: String) {
        if currentIndex < dialogueText.count {
            ...
            if gameState.currentDialog?.id == 22 {
                showCutscene()
            }
        ...
        for touch in touches {
            ...
            if currentIndex >= (gameState.currentDialog?.text.count)!{
                if gameState.currentDialog?.id == gameState.dialogTree.count - 1 {
                    ...
                    gameScene?.actNumber = 5
                    gameScene?.actTitle = "Epilogue"
                    gameScene?.nextScene = EpilogueScene(size: size)
                    gameScene!.scaleMode = .aspectFill
                    let transition = SKTransition.crossFade(withDuration: 1.0)
                    view?.presentScene(gameScene!, transition: transition)
                    return
