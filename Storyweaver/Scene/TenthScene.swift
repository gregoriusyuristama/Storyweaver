...
class TenthScene: SKScene {
    ...
    @ObservedObject private var gameState = GameState(dialogTree: DialogTree.DialogTreeScene10)
    ...
    override func didMove(to view: SKView) {
        ...
        backgroundNode = SKSpriteNode(imageNamed: "background_mbokSriniHouseYard")
        ...
        for touch in touches {
            ...
            if currentIndex >= (gameState.currentDialog?.text.count)!{
                if gameState.currentDialog?.id == gameState.dialogTree.count - 1 {
                    let gameScene = EleventhScene(size: size)
