...
class EleventhScene: SKScene {
    ...
    @ObservedObject private var gameState = GameState(dialogTree: DialogTree.DialogTreeScene11)
    ...
    override func didMove(to view: SKView) {
        ...
        cutsceneNode = SKSpriteNode(imageNamed: "cutscene_scene11_1")
        ...
        for touch in touches {
            ...
            if currentIndex >= (gameState.currentDialog?.text.count)!{
                if gameState.currentDialog?.id == gameState.dialogTree.count - 1 {
                    let gameScene = TwelfthScene(size: size)
                    gameScene.scaleMode = .aspectFill
                    let transition = SKTransition.crossFade(withDuration: 1.0)
                    view?.presentScene(gameScene, transition: transition)
                    return
