...
class ThirteenthAScene: SKScene {
    ...
    @ObservedObject private var gameState = GameState(dialogTree: DialogTree.DialogTreeScene13a)
...
//        addChild(characterLabel)
        ...
        cutsceneNode = SKSpriteNode(imageNamed: "cutscene_scene13a_1")
...
//        let mbokSrini = CreateEntity.mbokSriniEntity(scene: self, pos: .center)
        ...
        for touch in touches {
            ...
            if currentIndex >= (gameState.currentDialog?.text.count)!{
                if gameState.currentDialog?.id == gameState.dialogTree.count - 1 {
                    ...
                    gameScene?.actNumber = 4
                    gameScene?.actTitle = "The Day Leave"
                    gameScene?.nextScene = FourteenthScene(size: size)
